...
/// Reactive controller powered by RxDart [BehaviorSubject].
class ${pascal}Controller {
  ${pascal}Controller() {
    load();
  }

  final _state = BehaviorSubject<${pascal}State>.seeded(const ${pascal}State());
...
''',
  ...
  String _page(String feature, String pascal) {
    ...
    if (config.stateManagement == StateManagement.bloc ||
        config.stateManagement == StateManagement.cubit ||
        config.stateManagement == StateManagement.rxdart) {
      packageImports
      ...
      body: state.loading
          ...
          : ListView(
              ...
              children: [
                ${pascal}Header(title: state.title),
                const SizedBox(height: 16),
                const Text('Generated by stackchain'),
              ],
            ),
    );
  }
...
import '../controllers/${feature}_state.dart';
import '../widgets/${feature}_header.dart';
...
class _${pascal}PageState extends State<${pascal}Page> {
  late final ${pascal}Controller _controller;

  @override
  void initState() {
    super.initState();
    _controller = ${pascal}Controller();
  }
    ...
    return Scaffold(
      ...
      body: StreamBuilder<${pascal}State>(
        ...
        builder: (context, snapshot) {
          final state = snapshot.data ?? const ${pascal}State();
          return switch (state.status) {
            ${pascal}Status.initial || ${pascal}Status.loading =>
              const AppLoader(message: 'Loading...'),
            ${pascal}Status.failure => AppErrorWidget(
...
''';
  ...
  Map<String, String> _mvvm(
    ...
    if (config.stateManagement == StateManagement.rxdart) {
      return {
...
class ${pascal}ViewModel {
  ...
  final _model = BehaviorSubject<${pascal}Model?>.seeded(null);

  ValueStream<bool> get loading => _loading.stream;
  ValueStream<${pascal}Model?> get model => _model.stream;

  Future<void> load() async {
    _loading.add(true);
    await Future<void>.delayed(const Duration(milliseconds: 250));
    _model.add(const ${pascal}Model(title: '$pascal'));
    _loading.add(false);
  }
    ...
    return Scaffold(
      ...
      body: StreamBuilder<bool>(
        ...
        builder: (context, loadingSnap) {
          if (loadingSnap.data ?? false) {
            return const Center(child: CircularProgressIndicator());
          }
          return StreamBuilder<${pascal}Model?>(
            stream: _vm.model,
            builder: (context, modelSnap) {
              return Center(child: Text(modelSnap.data?.title ?? ''));
            },
          );
        },
      ),
